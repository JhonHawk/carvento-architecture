...
:::tip Corazón del Negocio
Este dominio representa el corazón del modelo de negocio de Carvento, donde cada vehículo pasa por un proceso controlado desde su ingreso hasta la venta final.
:::
...
graph TB
    ...
    Vehicle --> VehicleLead
    VehicleMedia --> VehiclePhoto

    classDef aggregateRoot fill:#e1f5fe
    classDef entity fill:#f3e5f5
    classDef valueObject fill:#e8f5e8

    class Vehicle aggregateRoot
    class VehicleLead entity
    class VehiclePhoto entity
    class VehicleSpecs,VehicleMedia,VehiclePricing,VehicleHistory,VehicleDocuments valueObject
```
...
:::info Patrón Aggregate Root
La entidad Vehicle actúa como Aggregate Root, controlando todas las modificaciones a sus entidades relacionadas y manteniendo la consistencia del dominio.
:::
...
```mermaid
stateDiagram-v2
    [*] --> PENDING_VALIDATION: Crear vehículo

    PENDING_VALIDATION --> AVAILABLE: Validación exitosa
    PENDING_VALIDATION --> WITHDRAWN: Rechazado

    AVAILABLE --> RESERVED: Cliente reserva
    AVAILABLE --> UNDER_MAINTENANCE: Mantenimiento
    AVAILABLE --> WITHDRAWN: Retirar

    RESERVED --> SOLD: Venta confirmada
    RESERVED --> AVAILABLE: Reserva expirada

    UNDER_MAINTENANCE --> AVAILABLE: Mantenimiento completo
    UNDER_MAINTENANCE --> WITHDRAWN: No reparable

    SOLD --> [*]: Proceso finalizado
    WITHDRAWN --> [*]: Proceso finalizado
```
...
:::warning Restricciones de Fotos
Un vehículo debe tener mínimo 5 fotos y máximo 20 fotos para garantizar una buena presentación sin sobrecargar el sistema.
:::
...
:::tip Pricing Inteligente
El sistema de pricing utiliza múltiples factores como datos de mercado de AutoMétricas, condición del vehículo y tendencias históricas para sugerir precios competitivos.
:::
...
La estructura sigue los principios de la arquitectura hexagonal (ports and adapters), separando claramente las preocupaciones de dominio, aplicación e infraestructura.
:::
...
:::warning Restricciones de Base de Datos
La base de datos incluye restricciones importantes como validación de VIN único, límites de millaje realista y garantía de una sola foto primaria por vehículo.
:::
...
:::info Reglas de Validación
- **VIN único**: Cada vehículo debe tener un VIN único en el sistema
- **Fotos mínimas**: Mínimo 5 fotos antes de marcar como disponible
- **Estados válidos**: Solo transiciones de estado permitidas según el diagrama
- **Millaje realista**: Validación de millaje basada en año del vehículo
:::

:::tip Gestión de Leads
- Los leads se crean automáticamente cuando un usuario muestra interés
- Los test drives se pueden agendar solo para leads activos
- Las notificaciones se envían tanto al vendedor como al comprador
:::

:::warning Reservas de Vehículos
- Un vehículo solo puede tener una reserva activa
- Las reservas expiran automáticamente después del tiempo configurado
- Solo vehículos disponibles pueden ser reservados
:::

## Referencias Relacionadas

- **[Base de Datos](../arquitectura/base-de-datos.md)**: Esquemas detallados de todas las tablas
- **[Arquitectura General](../arquitectura/arquitectura-general.md)**: Visión completa del sistema
- **Dominio de Usuarios**: Gestión de usuarios y autenticación (próximamente)
- **Dominio de Subastas**: Sistema de subastas en tiempo real (próximamente)
