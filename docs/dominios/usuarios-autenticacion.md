...
:::info Proceso de Verificación
La plataforma implementa un sistema de verificación progresiva que aumenta los privilegios del usuario según el nivel alcanzado.
:::
...
El agregado User es la entidad central del dominio, encapsulando toda la lógica de negocio relacionada con usuarios, autenticación y autorización.
...
La verificación de usuarios implementa múltiples capas de seguridad. Los documentos de identidad son procesados usando análisis automático con IA/ML y revisión manual cuando es necesario.
:::
...
:::tip Sistema de Roles Progresivo
Los roles en Carvento están diseñados para ser progresivos. Un usuario puede evolucionar desde visitante hasta vendedor premium basado en su nivel de verificación.
:::
...
sequenceDiagram
    ...
    U->>F: Formulario de registro
    F->>A: POST /auth/register
    A->>A: Validar datos
    A->>DB: Crear usuario
    A->>E: Enviar email verificación
    A->>F: Usuario creado
    F->>U: Confirmar registro

    Note over U,E: Usuario debe verificar email
    U->>E: Clic en link email
    E->>A: GET /auth/verify-email?token=xxx
    A->>DB: Marcar email verificado
    A->>F: Email verificado
    F->>U: Confirmación verificación
```
...
sequenceDiagram
    ...
    alt Requiere MFA
        A->>M: initiateMFA()
        M->>S: Enviar código
        A->>F: requiresMFA(mfaToken)
        F->>U: Solicitar código MFA

        U->>F: Ingresa código
        F->>A: completeMFA(token, code)
        A->>M: verifyCode()
        M->>A: código válido
        A->>F: AuthenticationResult.success()
    else Sin MFA
        A->>F: AuthenticationResult.success()
    end
...
sequenceDiagram
    ...
    participant AI as DocumentAnalysis
    participant Admin as Admin Review

    U->>F: Subir documentos
    F->>V: submitIdentityDocuments()
    V->>S3: Almacenar documentos (encriptados)
    V->>AI: Analizar documentos

    alt Análisis automático exitoso (confianza > 85%)
        AI->>V: Documentos válidos
        V->>V: aprobar automáticamente
        V->>F: Verificación aprobada
    else Requiere revisión manual
        AI->>V: Requiere revisión
        V->>Admin: Notificar revisión pendiente
        V->>F: En revisión

        Admin->>V: Aprobar/Rechazar
        V->>F: Resultado final
    end

    F->>U: Estado de verificación
```
...
Este servicio maneja toda la lógica de autenticación incluyendo MFA y gestión de sesiones.
...
Maneja el proceso completo de registro de usuarios con validaciones y notificaciones.
...
Gestiona el proceso de verificación de identidad incluyendo análisis automatizado de documentos.
...
:::note Diseño de Base de Datos
El esquema está optimizado para consultas de autorización frecuentes y auditoría de seguridad. Los índices están diseñados para soportar las operaciones más comunes.
:::
...
Este dominio implementa múltiples capas de seguridad para proteger contra ataques comunes como fuerza bruta, inyección SQL, y manipulación de sesiones.
:::

1. **Rate Limiting**: Prevención de ataques de fuerza bruta
2. **Account Lockout**: Bloqueo automático después de intentos fallidos
3. **Token Expiration**: Tokens JWT con tiempo de vida limitado
4. **Session Management**: Control de sesiones activas
5. **Audit Trail**: Registro completo de eventos de seguridad
6. **Input Validation**: Validación estricta de todos los inputs
7. **Encrypted Storage**: Documentos de identidad encriptados en S3

### Conformidad con Regulaciones

- **GDPR**: Derecho al olvido y portabilidad de datos
- **CCPA**: Privacidad del consumidor de California
- **SOX**: Auditoría y trazabilidad de cambios
- **PCI DSS**: Manejo seguro de información de pagos
...
- **Dominio de Leads** - Gestión de oportunidades de venta
- [Dominio de Vehículos](./inventario-vehiculos) - Inventario de vehículos
- [Arquitectura General](../arquitectura/arquitectura-general) - Patrones de seguridad generales
:::

---

Este dominio de Usuarios y Autenticación proporciona una base sólida de seguridad para toda la plataforma Carvento, implementando mejores prácticas de autenticación, autorización y verificación de identidad con un enfoque progresivo que permite a los usuarios acceder a más funcionalidades conforme aumenta su nivel de verificación.